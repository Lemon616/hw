...
def create_mask_from_points(points, img_h, img_w):
    ...
    Args:
        points (list): Polygon points of shape (n, 2).
    ...
    return mask * 255  # Convert to 0-255 mask
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    """
# Define a Laplacian kernel for each channel

    laplacian_kernel = torch.tensor([[0, 1, 0],
                                     [1, -4, 1],
                                     [0, 1, 0]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
    laplacian_kernel = laplacian_kernel.repeat(3, 1, 1, 1)  # 扩展到 3 个通道

    laplacian_kernel = laplacian_kernel.to(foreground_img.device)

    # Compute Laplacian for both images with groups=3 to match the 3 channels
    ...
    # Apply masks
    fg_masked = fg_laplacian * foreground_mask.unsqueeze(0).unsqueeze(0)
    blended_masked = blended_laplacian * background_mask.unsqueeze(0).unsqueeze(0)

    # Compute the Laplacian loss
    loss = F.mse_loss(fg_masked, blended_masked)
